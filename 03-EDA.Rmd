...
# Summary of the data
desc_summ <- describe(data[,2:ncol(data)])

# Summary of the net_score data
desc_score_summ <- describe(data_net_score[,2:ncol(data_net_score)])

# Converting Data from Wide to Long Form
# Factoring the non-reverse score responses
data_long <- data[,2:ncol(data)] %>%
  gather(everything(), key = "Qsn", value = "Resp")
data_long$Resp <- factor(data_long$Resp)

# Making Stacked Bar Plots for Grouped Qsns and Response
Qsn_Name <- c("^MEM.", "^ATTNPER.", "^SOC.", "^FAC.")
Plot_Nmaes <- c("Memory", "Attention & Perception", "Social Understanding","Facial Features")

Plot_Nmaes1 <- Plot_Nmaes
```
...
Before Looking into anything else, we see what the **average facial recognition ability score of the students is and looking at the 95% confidence interval** it seems the overall performance is very good standing at above 80%.
...
visualise(bootstrap_dist) +
  labs(
    x = "Average Face Recognition Ability Score",
    y = "Count"
  ) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(),
  ...
  ggtitle("95% C.I.-Bootstrap Distribution of Mean Facial Recognition Ability Score") +
...
suppressMessages(data_enc[,2:ncol(data_enc)] %>%
  ggcorr() +
  scale_fill_viridis() +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(),
...
data_net_score %>%
           ...
           ggpairs(columns = c("MEM", "ATTNPER", "SOC", "FAC"), lower = list(continuous = my_fn), progress = FALSE) +
...
```{r corr_plot_each_item_each_domain, fig.asp=0.8, fig.width=14}
# Tile Plots for correlation for each Qsn in each Category
Qsn_Name_Cont <- c("MEM", "ATTNPER", "SOC", "FAC")
...
```{r fit_pdfs, fig.asp=1.6, fig.width=16, warning=FALSE}
...
proper_fit_distribution <- function(data, heading = "Model & Data"){
  ...
  if(list(fit_g, fit_l, fit_w, fit_n)[as.numeric(which.min(gof$ks))][[1]]$distname == "weibull")
  ...
  if(list(fit_g, fit_l, fit_w, fit_n)[as.numeric(which.min(gof$ks))][[1]]$distname == "lnorm")
  ...
  if(list(fit_g, fit_l, fit_w, fit_n)[as.numeric(which.min(gof$ks))][[1]]$distname == "gamma")
  ...
  if(list(fit_g, fit_l, fit_w, fit_n)[as.numeric(which.min(gof$ks))][[1]]$distname == "norm")
  ...
  if(list(fit_g, fit_l, fit_w, fit_n)[as.numeric(which.min(gof$ks))][[1]]$distname == "exp")
...
fit_pdf <- list()
for (i in seq_along(data_net_score[,2:ncol(data_net_score)])) {
  fit_pdf[[i]] <- annotate_figure(
    proper_fit_distribution(data_net_score[[i+1]]),
...
```{r each_item_each_domain, fig.asp=0.8, fig.width=14}
# Mean and CI of each Qsn for each item

desc_sum <- describe(data_enc[,2:ncol(data_enc)])
...
```{r boxplots, fig.asp=0.618, fig.width=14}
# Boxplots
...
boxplot1 <- data_tot_score %>%
  ...
  labs(
    x = "Face Recognition Ability Score",
    y = "Sex"
  ) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), panel.grid.major = element_blank(),
...
Cat_Score_aov <- aov(Score ~ Category, data = data_net_score[,2:ncol(data_net_score)] %>%
                       gather(everything(), key = "Category", value = "Score") %>%
                       mutate(Category = as.factor(Category)) %>%
                       filter(Category %in% c("MEM", "FAC", "SOC")))

Cat_Score_aov <- present_aov.test(Cat_Score_aov, footnote = "One-Factor Anova for MEM, FAC, SOC Scores", digs = 2)


tests <- ggarrange(gen_v_tot_score_t_test, Cat_Score_aov, ncol = 2, nrow = 1)

test_plots <- ggarrange(boxplot1, boxplot3, ncol = 2, nrow = 1)

p <- ggarrange(test_plots, tests, ncol = 1, nrow = 2, heights = c(10, 2))

annotate_figure(
  p,
  top = text_grob("Face Recogntion Ability Category Scores and Total Scores", face = "bold", size = 16)
)
...
lin_mod_cats <- Qsn_Name_Cont %>%
  map(~str_c(.,"~.")) %>%
  map(~lm(as.formula(.), data = data_net_score[,2:ncol(data_net_score)]))
...
```{r item_on_domain_LR, fig.asp=0.618, fig.width=14}
# Linear Model Dependence for each item to the Category net score

data_enc_1 <- data_enc %>%
  bind_cols(data_net_score[,2:ncol(data_net_score)])

lin_mod_items_on_cats <- list()
Qsn_Name1 <- c("^MEM", "^ATTNPER", "^SOC", "^FAC")
for (i in seq_along(Qsn_Name1)) {
  lin_mod_items_on_cats[[i]] <- data_enc_1[,2:ncol(data_enc_1)] %>%
...
```{r domain_on_item_LR, fig.asp=0.618, fig.width=14}
# Linear Model Dependence for each item to the Category net score
...
lin_mod_cats_on_tot <- Qsn_Name_Cont %>%
  map(~str_c("TOT~",.)) %>%
  map(~lm(as.formula(.), data = data_net_tot_score[,2:ncol(data_net_tot_score)]))
...
sig_lin_mod_cats_on_tot_plot <- data_net_tot_score %>%
  gather(MEM, ATTNPER, SOC, FAC, key = "Domains", value = "Score") %>%
...
```{r corr_dendo_only, fig.asp=0.618, fig.width=14, warning=FALSE, message=FALSE}
data_enc_trans <- as_tibble(t(data_enc[,2:ncol(data_enc)])) %>%
  add_column("Qsn" = colnames(data_enc[,2:ncol(data_enc)])) %>%
...
A2Rplot(
  ...
  show.labels=T,
  main="Correlation Based Similar Item Clustering"
)
```
...
```{r jacc_qsn_per, fig.asp=1, fig.width=14}
...
heatmap_dendo_plot <- function(qsn_dist = "euclidean", per_dist = "euclidean") {
  ...
  hclust.row <- hclust(dist(data_enc[,2:ncol(data_enc)], method = per_dist)) # clustering of persons(rows)
  heatmap.2(mat, col = bluered(100), 
            trace = "none", density.info = "none",
            Colv = as.dendrogram(hclust.row),
            Rowv = as.dendrogram(hclust.col),
          main = "Maximum & Maximum Distance Based Clustering of Domains and Students"
  )
}

# Person-Euclidean, Qsn-Euclidean
heatmap_dendo_plot("maximum", "maximum")
```
...
```{r corr_dendo_only_dom, fig.asp=0.618, fig.width=14}
# Pairwise correlation between samples (columns)
cols.cor <- cor(data_net_score[,2:ncol(data_net_score)], use = "pairwise.complete.obs", method = "pearson")
...
A2Rplot(
  ...
  col.down = c("#ff9900", "#4ECDC4", "#556270", "#ff66ff", "#00cc00", "#cc0000", "#cccc00", "#ADFF2F", "#000080"),
  show.labels=T,
  main="Correlation Based Similar Domain Clustering"
)
```
...
```{r max_qsn_euc_per, fig.asp=1, fig.width=14}
# Domain Clustering
# Trying Euclidean Distance based clustering
data_net_score_trans <- as_tibble(t(data_net_score[,2:ncol(data_net_score)])) %>%
  add_column("Domain" = colnames(data_net_score[,2:ncol(data_net_score)])) %>%
...
heatmap_dendo_plot <- function(qsn_dist = "euclidean", per_dist = "euclidean") {
  ...
  hclust.row <- hclust(dist(data_net_score[,2:ncol(data_net_score)], method = per_dist)) # clustering of persons(rows)
  heatmap.2(mat, col = bluered(100), 
            trace = "none", density.info = "none",
            Colv = as.dendrogram(hclust.row),
            Rowv = as.dendrogram(hclust.col),
          main = "L2 & L2 Distance Based Clustering of Domains and Students"
  )
}
...
```{r pca_dom_level, fig.asp=1, fig.width=14}
...
data_long_spread2 <- data_long_spread %>%
  ...
  group_by(Domain) %>%
  summarise(
    `Strongly Disagree` = sum(`1`), 
    Disagree = sum(`2`),
    Neutral = sum(`3`),
    Agree = sum(`4`),
    `Strongly Agree` = sum(`5`)
  )
...
```{r pca_item_level, fig.asp=1, fig.width=14}
...
```{r rel_All, fig.asp=1.2, fig.width=16, warning=FALSE}
present_alpha <- function(alpha_mod, heading = NULL, visual = TRUE) {
  t1 <- as_tibble(alpha_mod$total) %>% 
    mutate(across(where(is.numeric), ~ round(., digits = 2)))
  t2 <- conv_to_tibble(alpha_mod$alpha.drop)
...
all_item_alpha <- present_alpha(alpha(data_enc[,2:ncol(data_enc)], check.keys = TRUE), "Reliability Analysis of Face Recognition Ability Response Data")
all_item_alpha
```
